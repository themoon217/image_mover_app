...
4.Pythonスクリプトの実行 file_mover.py を実行することでアプリが起動します。コマンドプロンプトまたはターミナルで以下を実行してください：

python file_mover.py

使い方
1.アプリケーションを起動すると、まず移動元のフォルダを選択するダイアログが表示されます。ここでは画像ファイルが入ったフォルダを指定します。
2.次に移動先のフォルダを選択します。
3.次に、画像ファイル名が書かれたテキストファイルを選択します。このファイルには、移動する画像の名前（拡張子なし）を一行ずつ書いてください。
4.アプリは、リストに書かれた名前の画像を移動元から移動先へと移動します。
